...
public class ExcludesExpression implements BooleanExpression {
	...
	public ExcludesExpression(String longID1, String longID2, FeaturesSymbolTable featuresSymbolTable) {
	...
	public int getType() throws AmbiguousReferenceException, IllegalExpressionException, SymbolNotFoundException, ChildrenFeatureNotFoundException, SharedFeatureUsingParentSelectorException {
		if (featuresSymbolTable.containsSymbol(longID1)) {
			this.normalFormLongID1 = (String) featuresSymbolTable.getSymbol(longID1).get(1);
			if (featuresSymbolTable.containsSymbol(longID2)) {
				this.normalFormLongID2 = (String) featuresSymbolTable.getSymbol(longID2).get(1);
				return Expression.BOOL;
			}
			else {
				throw new IllegalExpressionException("Type error : the expression "+this.toString()+" is invalid. The right paramater ( "+longID2+" ) is not a valid feature ID. In an excludes expression, each parameter must be a valid feature ID.");
			}
		}
		else {
			throw new IllegalExpressionException("Type error : the expression "+this.toString()+" is invalid. The let paramater ( "+longID1+" ) is not a valid feature ID. In an excludes expression, each parameter must be a valid feature ID.");
		}
	}
	
	public Expression toNormalForm() {
		return new ExcludesExpression(this.normalFormLongID1, this.normalFormLongID2, null);
	}
	
	public BooleanExpression toSimplifiedForm() {
		return new OrExpression(new NotExpression(new LongIDExpression(this.longID1, this.featuresSymbolTable)), new NotExpression(new LongIDExpression(this.longID2, this.featuresSymbolTable)));
	}
