...
public class LEQExpression implements BooleanExpression {

	Expression expression1, expression2;

	public LEQExpression(Expression e1, Expression e2) {
			...
			if (this.expression2.getType() == Expression.INT) {
				...
				if (this.getExpression2().getType() == Expression.REAL) {
					...
					throw new IllegalExpressionException("Type error : the expression " + this.toString()
							...
							+ " ) of a lower or equal expression must be real or int. Currently,  its type is "
	...
	public Expression toNormalForm() {
		return new LEQExpression(this.expression1.toNormalForm(), this.expression2.toNormalForm());
	}
		...
		if ((londIDExpression1.getSymbol() != null) && (londIDExpression2.getSymbol() != null)) {
			...
			BooleanExpression simplifiedExpression = new ImpliesExpression(new LongIDExpression(londIDExpression2.getNormalizedLongID()
					...
					+ enumAttribute2.getBooleanAttribute(
							...
							.getID(), null), new LongIDExpression(londIDExpression1.getNormalizedLongID()
					+ "."
					+ enumAttribute1.getBooleanAttribute(
							((EnumSetExpressionSymbol) enumAttribute1.getSetExpressionSymbol()).getContainedValues().get(i).toString())
							.getID(), null));
			i++;
			while (i <= enumAttribute1.getNumberOfBooleanAttributes() - 2) {
				int j = 0;
				OrExpression orExpression = new OrExpression(new LongIDExpression(londIDExpression1.getNormalizedLongID()
						+ "."
						+ enumAttribute1.getBooleanAttribute(
								((EnumSetExpressionSymbol) enumAttribute1.getSetExpressionSymbol()).getContainedValues().get(j).toString())
								.getID(), null), new LongIDExpression(londIDExpression1.getNormalizedLongID()
						+ "."
						+ enumAttribute1.getBooleanAttribute(
			...
			if (londIDExpression1.getSymbol() != null) {
				...
				if (i == enumAttribute1.getNumberOfBooleanAttributes() - 1) {
