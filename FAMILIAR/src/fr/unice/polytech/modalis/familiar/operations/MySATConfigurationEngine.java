 ...
 * @author macher
 *
 * basically a copy & paste due to the lack of extension in SATConfigurationEngine
 */
public class MySATConfigurationEngine extends ConfigurationEngine {

	
	protected static Logger _LOGGER = Logger.getLogger(MySATConfigurationEngine.class);
	
	protected FMReasoningWithSAT reasoner;		// SAT solver
			
	public MySATConfigurationEngine(FeatureModel fm)	throws ConfigurationEngineException {
		super (fm);		
		...
		try {
			...
			_LOGGER.debug("Now loading the solver...");
	...
	public synchronized ConfigurationStep autoComplete(boolean valueOrder) throws ConfigurationEngineException {
		...
		try {
			...
			//reasoner.setVariableOrderObject(satOrderObj);
			reasoner.setVariableOrder(satVarOrder);
