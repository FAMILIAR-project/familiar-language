...
public class ChocoReasoner extends FeatureModelReasoner {
	...
	protected Map<Pair, Map<String, IntegerVariable>> variables;
	...
	private void initPair() {
		for(Pair p:pairs){
			...
			setRelations.put(p, new HashMap<String, IntegerExpressionVariable>());
		}
	}

	public void addPairWiseConstraints(){
		for(Pair p:pairs){
			Constraint c = and(eq(1,variables.get(p).get(p.featurea.getName())),eq(1,variables.get(p).get(p.featureb.getName())));
	...
	protected void createFeature(Feature f,
			Collection<Cardinality> cards, Pair p) {
			...
			while (it.hasNext()) {
				...
				if (d instanceof IntegerDomain) {
					...
					if (d instanceof RangeIntegerDomain) {
						...
						Iterator<Range> itRanges = rangeDom.getRanges()
								.iterator();
						...
						attVar = makeIntVar(
								attName + "_" + p.featurea.getName() + "+"
										+ p.featureb.getName(), min, max,
								"cp:bound", "cp:no_decision");
						...
						attVar = makeIntVar(
								attName + "_" + p.featurea.getName() + "+"
										+ p.featureb.getName(), valsArray,
								"cp:enum", "cp:no_decision");
					}
...
//	}
	...
	protected Constraint createMandatory(Relation rel,
			Feature child, Feature parent, Pair p) {
	...
	protected Constraint createCardinality(Relation rel,
			Feature child, Feature parent,
			Iterator<Cardinality> cardinalities, Pair p) {
	...
	protected Constraint createRequires(Relation rel,
			Feature origin, Feature destination, Pair p) {
	...
	protected Constraint createExcludes(Relation rel,
			Feature origin, Feature dest, Pair p) {
