...
public class PacogenReasoner extends FeatureModelReasoner {
	...
	public void addExcludes(Relation rel, Feature origin, Feature destination) {
	...
	public void addFeature(Feature feature, Collection<Cardinality> cardIt) {
	...
	private String initVariable(Feature Feature) {
		if (FamaToPacoName.containsKey(Feature.getName())) {
			...
			VarAttrList.add("put_atts(" + PacoName + ",featureName('"
					+ Feature.getName() + "'))");
			FamaToPacoName.put(Feature.getName(), PacoName);
			AttributesName
					.put(Feature.getName(), new HashMap<String, String>());
			ftList.add(PacoName);
	...
	private void initAttributes(Feature Feature) {
		...
		Iterator<? extends GenericAttribute> it = Feature.getAttributes()
				.iterator();
		...
		while (it.hasNext()) {
			...
			String plName = FamaToPacoName.get(Feature.getName()) + "_"
					+ name.toUpperCase();
			if (att.getDefaultValue() instanceof Integer) {
				if ((Integer) att.getDefaultValue() != 0) {
					...
					ctrModel.add("(" + FamaToPacoName.get(Feature.getName())
							+ ") #= 0 #<=> (" + plName + "#= 0)");
					ctrModel.add("(" + FamaToPacoName.get(Feature.getName())
							+ ") #\\= 0 #<=> (" + plName + "#\\= 0)");
					String ListName = "L" + plName + " = [0,"
							+ att.getDefaultValue() + "]";
	...
	public void addSet(Relation rel, Feature parent,
		...
		if (cardinalities.size() > 1) {
			...
			childrenNameList = childrenNameList.substring(0,
					childrenNameList.length() - 1)
					+ "]";
			...
			if (card.getMin() == 1 && card.getMax() == children.size()) {
				...
				ctrModel.add("card(" + card.getMin() + "," + card.getMax()
						+ "," + FatherName + "," + childrenNameList + ")");
			}
		}
	...
	public void buildModel() throws IOException {
		...
		String Output = "[reason," + StrctrModel + "," + StrftList + ","
				+ StrattrDomain + "," + StrAttrPrologNameList + "].";
		fw.write(Output);
		fw.close();
	}
	...
	public String getVarDomain() {
		...
		for (int j = 0; j < AttrListFamaName.size(); j++) {
			res += AttrListFamaName.get(j) + ": "
					+ domainList.get(ftListFamaName.size() + j) + "\n";
		}
	...
	protected class AstParser {
		...
		private String translateLogical(Node<String> tree) {
			...
			if (n == 2) {
				if (data.equals(KeyWords.AND)) {
					System.err
							.println("Pacogen doen't handle logical relations yet");
		...
		private String translateInteger(Node<String> tree) {
			...
			if (data.equals(KeyWords.PLUS)) {
				...
				// TODO
				// Integer i =
				// constantIntConverter.translate2Integer(tree.getData());
				// Restmp += i.toString() ;
			}
